...
const todoValidation = ({todo}) =>{
    return new Promise((resolve, reject) => {
        if (!todo) reject("Missing todo");
        if (typeof todo!== "string") reject("todo is not a text");
        if (todo.length < 3 || todo.length > 100) reject("Length must be between 3 and 100");
...
const sendVerificationMail = (email, token) => {
    ...
    const transpoter = nodemailer.createTransport({
      ...
      service: "gmail",
      auth: {
        user: "ncrail27@gmail.com",
        pass: "onge femf kcyt xxae",
      },
    });
  
    //mail options
    const mailOptions = {
      from: "ncrail27@gmail.com",
      to: email,
        ...
        <td bgcolor="#ffffff" style="padding: 40px 30px 40px 30px;">
          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse;">
            ...
            <tr>
              ...
              <a href="https://task-manager-node-2ig8.onrender.com/verifytoken/${token}" style=" border: none;
...
module.exports = {todoValidation, generateToken, sendVerificationMail};